...
try:
    ...
    print(
        "⚠️ Whisper/torch not available. Install with: pip install openai-whisper torch"
    )
...
try:
    ...
    classifier = AimeeClassifier(
        data_file="aimee_training_data_tagged.json", threshold=0.2
    )
        ...
        with self.lock:
            if model_type not in self.models:
                try:
                    ...
                    self.models[model_type] = whisper.load_model(
                        model_type, device=self.device
                    )
...
def extract_key_details(transcript: str, intent: str) -> str:
    ...
    for cust_key, cust_name in customers.items():
        ...
        if re.search(rf"\b{re.escape(cust_key)}\b", text):
            customer = cust_name
            break
    ...
    if "charchoos green" in text:
        ...
        match = re.search(
            r"(château|chateau)\s+([a-z\s]+?)(?:\s|,|\.|$)", text, re.IGNORECASE
        )
    ...
    if intent == "add_to_order":
        ...
        if wine:
            ...
            return "I'm your Fairfield County wine intelligence. I brief you on 17 accounts, track daily priorities, analyze gaps and opportunities. I turn  voice commands into tactical advantage. Try: Brief me on Spiga, Daily priorities, Market analysis."
    ...
    if intent in [
        ...
        print(f"DEBUG - Calling get_customer_intelligence for {intent} with: '{text}'")
        try:
            response = get_customer_intelligence(text)
            print(f"DEBUG - Got intelligence response: {response[:100]}...")
            return response  # Return the intelligence directly (no "I heard:" prefix)
        except Exception as e:
            print(f"DEBUG - Customer intelligence error for {intent}: {e}")
            import traceback

            traceback.print_exc()
            return f"I heard: {text}. Let me check the tactical intelligence data."
        try:
            response = get_customer_intelligence(
                customer
            )  # Pass just the customer name
            print(f"DEBUG - Got intelligence response: {response[:100]}...")
            return response
        except Exception as e:
        ...
        if wine_intelligence and hasattr(
            wine_intelligence, "get_wine_recommendations_by_preference"
        ):
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                audio = elevenlabs_client.text_to_speech.convert(
                    voice_id=voice_id, text=text, model_id="eleven_monolingual_v1"
                )
...
def verify_transcript_background(filepath: str, base_transcript: str, file_hash: str):
    ...
    def _verify():
        try:
            ...
            if (
                len(medium_transcript) > len(base_transcript) * 0.8
            ):  # Reasonable length check
                confidence_boost = min(
                    0.2,
                    abs(len(medium_transcript) - len(base_transcript))
                    / len(base_transcript),
                )
                print(
                    f"Background verification complete. Confidence boost: +{confidence_boost:.2f}"
                )
                ...
                verification_log = os.path.join(
                    app.config["UPLOAD_FOLDER"], "verification_log.json"
                )
                try:
...
def test_classification():
    ...
    try:
        ...
        if wine_intelligence and hasattr(
            ...
            try:
                wine_recs = wine_intelligence.get_wine_recommendations_by_preference(
                    text, limit=3
                )
                wine_test_results = {
        ...
        try:
            ...
            fairfield_test_results = {
                ...
                "response_preview": (
                    fairfield_response[:100] + "..."
                    if len(fairfield_response) > 100
                    else fairfield_response
                ),
                "contains_tactical_data": any(
                    keyword in fairfield_response.lower()
                    for keyword in ["tactical", "annual volume", "gaps", "lead with"]
                ),
            }
        except Exception as e:
...
def test_wine_intelligence():
    ...
    try:
        ...
        recommendations = wine_intelligence.get_wine_recommendations_by_preference(
            query, limit=5
        )
        ...
        taste_descriptors = [
            ...
            if word in ["fruity", "bold", "crisp", "smooth", "light", "dry"]
        ]
        taste_response = (
            wine_intelligence.handle_taste_query(taste_descriptors)
            if taste_descriptors
            else "No taste descriptors found"
        )
    ...
    try:
        if not model_manager.is_available("base"):
            return (
                jsonify(
                    {
                        "error": "Whisper base model is not available. Please install openai-whisper."
                    }
                ),
                500,
            )
        ...
        try:
            ...
            # Start logging (async)
            executor.submit(log_interaction, transcript, classification, "pending")

            # Wait for audio generation (with timeout)
            try:
                audio_filename = audio_future.result(
                    timeout=15
                )  # 15 second timeout for ElevenLabs
            except Exception as e:
            ...
            response_data = {
                ...
                "verification_started": use_verification
                and model_manager.is_available("medium"),
        ...
        return jsonify(
            {
                ...
                "transcription_time": transcript_cache[medium_cache_key].get(
    ...
    def _cleanup():
        try:
            ...
            for filename in os.listdir(app.config["UPLOAD_FOLDER"]):
                if (
                    filename.startswith("response_")
                    or filename.startswith("aimee_response_")
...
if __name__ == "__main__":
    ...
    print(
        f"Enhanced Wine Intelligence: {'✅ Ready' if wine_intelligence and hasattr(wine_intelligence, 'get_stats') else '❌ Not available'}"
    )
    ...
    print(
        f"Accounts: Barcelona (2), Spiga, ELM, The Cottage, Bin 100, Blackstones (3), Rebecca's, 99 Bottles, Horseneck, DB Fine Wines, Greens Farms, LaBella's, Acme"
    )
    print(
        f"Voice Commands: 'Brief me on [account]', '[Region] accounts', 'Daily priorities', 'Market analysis'"
    )
